  ...
  const danmakuActionMenu = (uid: number, uname: string, text: string) => {
    return (
      <div className="danmakuActionMenu">
        <span
          className="danmakuActionMenuUser pointer"
        ...
        <span
          className="danmakuActionMenuItem pointer"
          onClick={() => handleReadDanmaku(uname, text)}
        >
          {t('DanmakuRead')}
        </span>
        <span
        ...
        <span
          className="danmakuActionMenuItem pointer"
          onClick={() => handleBlockDanmaku(text)}
        >
          {t('DanmakuBlockSimilar')}
        </span>
      </div>
    )
  }
  ...
  return (
    <div
      className={[
        'danmakuItem',
        props.guardLevel
          ? `danmakuItem danmaku-item ${
              config.blockEffectItem5 === 0
                ? `guard-level-${props.guardLevel}`
                : ''
            } guard-danmaku`
          : '',
      ].join(' ')}
    >
      ...
      <i
        className={[
          'v-middle bg-center bg-no-repeat pointer',
          props.guardLevel
            ? `dp-i-block guard-icon guard-level-${props.guardLevel}`
            : '',
        ].join(' ')}
      />
      <MsgVip {...props} />
      {props.fanLv && config.showFanLabel === 1 && (
        <div className="fans-medal-item-ctnr dp-i-block p-relative v-middle">
          <div
            className={[
              props.fanLv &&
                `fans-medal-item level-${props.fanLv > 20 ? 20 : props.fanLv}`,
            ]}
          >
      ...
      {config.showLvLabel === 1 && (
        <div
          className={`user-level-icon dp-i-block p-relative v-middle ${
            props.userLevel && `lv-${props.userLevel}`
          }`}
        >
          UL {props.userLevel}
        </div>
      ...
      <span
        className={`user-name v-middle pointer open-menu ${
          props.guardLevel ? 'guard' : ''
        }`}
      >
        <UserWrapper>{props.username}:</UserWrapper>
      </span>
      <Tooltip
        visible={showToolTip}
        animation="zoom"
      ...
      >
        <span className="danmaku-content v-middle pointer ts-dot-2 open-menu">
          <ContentWrapper>
            {translateContent}
            {props.repeat > 0 && (
              <span className={`repeatNum repeat-num-${props.repeat}`}>
                {props.repeat}
              </span>
            )}
          </ContentWrapper>
