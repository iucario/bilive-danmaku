...
function MsgDanmuIcon(props: DanmakuIcon) {
  ...
  return (
    <div className={["danmakuItem", props.guardLevel ? `danmakuItem danmaku-item ${config.blockEffectItem5 === 0 ? `guard-level-${props.guardLevel}` : '' } guard-danmaku` : ''].join(' ')}>
      ...
      <Tooltip
        ...
        trigger="click"
        overlay={danmakuActionMenu(props.userID, props.username, props.iconName)}
      >
        <span className="v-middle pointer ts-dot-2 open-menu">
          <ContentWrapper>
            <img src={ props.iconUrl }
                 alt={ props.iconName }
                 title={ props.iconName }
                 style={ { width: props.width, height: props.height } }
                 className="danmakuIcon" />
            { props.repeat > 0 && (
              <span className={ `repeatNum repeat-num-${ props.repeat }` }>{ props.repeat }</span>) }
          </ContentWrapper>
        </span>
      </Tooltip>
      </div>
...
export default memo(MsgDanmuIcon);
